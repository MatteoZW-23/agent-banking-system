    ...
    <div className="min-h-screen w-full flex bg-gray-50">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-[420px]">
          ...
          {/* Role selector */}
          <div className="mb-8 grid grid-cols-2 md:grid-cols-4 gap-1.5 p-1.5 bg-gray-100 rounded-xl">
            {ROLES.map((r) => (
              <button
                key={r.value}
                type="button"
                onClick={() => setRole(r.value)}
                className={`h-10 rounded-lg text-xs font-bold transition-all ${role === r.value
