...
export function ProviderConfigContent() {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'fintech': return <Globe className="w-5 h-5" />;
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
       case 'mobile_money': return 'bg-blue-500/10 text-blue-600';
       case 'bank': return 'bg-emerald-500/10 text-emerald-600';
       case 'fintech': return 'bg-indigo-500/10 text-indigo-600';
    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2 border-b border-slate-200 dark:border-slate-800">
        <div className="space-y-1.5">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            Configure and maintain security credentials for all integrated providers.
          </p>
        </div>
        <Button onClick={() => setShowForm(!showForm)} className="h-11 rounded-xl premium-gradient text-white px-6 font-bold shadow-lg shadow-primary/20 transition-all hover:scale-105 active:scale-95">
      ...
      {showForm && (
        <Card className="border-none shadow-2xl dark:bg-slate-900/80 overflow-hidden ring-1 ring-primary/20 animate-fade-in">
          ...
          <CardHeader className="pb-8">
            ...
            <CardDescription>Establishing a secure link to a third-party banking API</CardDescription>
          </CardHeader>
          <CardContent className="space-y-8 max-w-3xl">
            <div className="grid gap-6 md:grid-cols-2">
               <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Friendly Name</label>
                  <input
                    type="text"
                    value={formData.name}
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Gateway Classification</label>
                  <select
                    value={formData.type}
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800">
              <Button variant="ghost" onClick={() => setShowForm(false)} className="h-11 px-8 rounded-xl font-bold uppercase tracking-widest text-xs">
                Cancel
              </Button>
              <Button onClick={handleSave} className="h-11 px-8 rounded-xl premium-gradient text-white font-bold shadow-lg shadow-primary/20">
                Save Configuration
              </Button>
            </div>
          </CardContent>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {providers.data?.map((provider) => (
          <Card key={provider.id} className="hover-lift border-none shadow-sm dark:bg-slate-900/50 group overflow-hidden">
            <CardContent className="pt-8 relative">
               <div className="absolute top-0 right-0 p-4">
                  <Badge variant="outline" className={`border-none ${provider.isActive ? 'text-emerald-500' : 'text-slate-300'} font-bold text-[10px] uppercase flex items-center gap-1.5`}>
                     <CircleDot className={`w-2 h-2 ${provider.isActive ? 'animate-pulse bg-emerald-500' : 'bg-slate-300'} rounded-full`} />
                     {provider.isActive ? 'Live' : 'Inactive'}
                  </Badge>
               </div>

               <div className="flex flex-col items-start gap-5">
                  <div className={`p-4 rounded-2xl ${getCategoryColor(provider.category)}`}>
                  ...
                  <div className="w-full space-y-4 pt-2">
                     <div className="p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mb-1.5">Primary API Cluster</p>
                        <div className="flex items-center justify-between text-xs font-mono text-slate-600 dark:text-slate-300">
                           <span className="truncate">{(provider as any).apiEndpoint || 'CLUSTER_01.AWS.PROD'}</span>
                           <ExternalLink className="w-3 h-3 shrink-0 ml-2 opacity-40" />
                        </div>
                     </div>

                     <div className="flex gap-2">
                       <Button variant="outline" size="sm" className="flex-1 h-9 rounded-lg border-slate-200 font-bold text-[10px] uppercase" onClick={() => setEditingId(provider.id)}>
                         <Settings className="w-3 h-3 mr-2" /> Settings
                       </Button>
                       <Button variant="outline" size="sm" className="h-9 w-9 rounded-lg border-slate-200 text-rose-500 hover:bg-rose-50 hover:text-rose-600 transition-colors">
        ...
        {!showForm && (
          ...
          >
             ...
             <div className="text-center">
               <p className="font-bold text-slate-800 dark:text-white">Provision New Gateway</p>
               <p className="text-xs text-slate-500 mt-1">Connect another regional banking node</p>
             </div>
          </button>
        )}
      </div>
      ...
      {providers.data?.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 text-center bg-white dark:bg-slate-900/50 rounded-3xl border-2 border-slate-50 dark:border-slate-800">
           ...
           <h3 className="text-2xl font-bold text-slate-800 dark:text-white">Registry Empty</h3>
           <p className="text-sm text-slate-500 max-w-sm mt-2 font-medium">No operational gateways detected. Start by provisioning your first mobile money or bank integration.</p>
           <Button className="mt-8 h-12 px-8 rounded-xl premium-gradient text-white font-bold shadow-xl shadow-primary/20" onClick={() => setShowForm(true)}>
